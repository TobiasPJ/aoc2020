defmodule Q2 do
  def checkAmountOfchar(c, word) do
    freq = word |> String.graphemes |> Enum.frequencies
    freq[c]
  end

  def count do
    entries = parsePasswords()
    Enum.count(entries, fn e -> parseEntrie(e) end)
  end


  def parsePasswords() do
    str = ~S[13-15 c: cqbhncccjsncqcc
    2-3 v: zvdvfd
    ...
    Enum.chunk_every(split_string, 3) #creates a list of lists with 3 entires

  end

  def parseEntrie(entrie) do
    pos_nums =  String.split(Enum.at(entrie, 0),"-")

    num1 = pos_nums |> Enum.at(0) |> String.to_integer() |> Kernel.-(1)
    num2 = pos_nums |> Enum.at(1) |> String.to_integer() |> Kernel.-(1)
    char = Enum.at(entrie, 1) |> String.trim(":")
    password = Enum.at(entrie, 2)

    #amount = checkAmountOfchar(char, password) #for challenge one

    checkpolicy(num1, num2, char, password)


  end

  def checkpolicy(num1, num2, char, password, chalnum \\ :two, amount \\ 0) do
    if chalnum == :one do
      if amount >= num1 && amount <= num2 do
        true
      else
        false
      end
    else
      #String.at(password, num1) == char XOR String.at(password, num2) == char
      if (String.at(password, num1) == char || String.at(password, num2) == char) && !(String.at(password, num1) == char && String.at(password, num2) == char) do
        true
      else
        false
      end
