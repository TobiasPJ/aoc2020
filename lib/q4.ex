defmodule Q4 do
  ...
  def solve do
    all_passengers = parsePassengers()

    Enum.count(all_passengers, fn passenger -> checkConditions(passenger) end)
  end

  def checkConditions(passenger) do
    eyr = "eyr:"
    iyr = "iyr:"
    byr = "byr:"
    hgt = "hgt:"
    ...
    if String.contains?(passenger, eyr) && String.contains?(passenger, iyr) && String.contains?(passenger, byr) && String.contains?(passenger, hgt) && String.contains?(passenger, hcl) && String.contains?(passenger, ecl) && String.contains?(passenger, pid) do
      checkAdditionalRules(passenger)
    else
      false
    end
  end

  def checkAdditionalRules(passenger) do
    #we know that all required fields exist when we come in here
    cleaned_string = String.replace(passenger,"\n", "")
    data = String.split(cleaned_string)

    Enum.all?(data, fn x -> ruleChecker(x) end)
  end

  def ruleChecker(field) do
    split = String.split(field, ":")
    type = Enum.at(split, 0)
    data = Enum.at(split, 1)

    case type do
      "byr" -> String.length(data) == 4 && String.to_integer(data) in 1920..2002
      "iyr" -> String.length(data) == 4 && String.to_integer(data) in 2010..2020
      "eyr" -> String.length(data) == 4 && String.to_integer(data) in 2020..2030
      "hgt" -> hgtCond(data)
      "hcl" -> String.match?(data, ~r/[#[:alnum:]]/) && String.length(data) == 7
      "ecl" -> String.contains?(data, ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"])
      "pid" -> String.length(data) == 9
      "cid" -> true
      end
  end

  def hgtCond(data) do
    if String.contains?(data, "cm") do
      temp = String.replace(data, "cm", "")
      h = String.to_integer(temp)
      h in 150..193
    else
      temp = String.replace(data, "in", "")
      h = String.to_integer(temp)
      h in 59..76
    end
  end

  def parsePassengers do
    passengers = "eyr:2029 byr:1931 hcl:z cid:128
