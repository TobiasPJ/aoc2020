defmodule Q7 do
  ...
  def getBagTypes(map) do
    Map.keys(map)
  end

  def getContain(map, bag) do
    if String.last(bag) != "s" do
      bag = bag <> "s"
      map[bag]
    else
      map[bag]
  ...
  def getNumberOfBags(map, bag) do
    contain = getContain(map, bag)
    bags = getBagNamesFromContain(contain)
    numbers = getNumbersFromContain(contain)

    num_bag_list= getBagNumList(numbers, bags)

    if num_bag_list != nil do
      Enum.reduce(num_bag_list, 0, fn row, acc -> acc + Enum.at(row,0) * getNumberOfBags(map, Enum.at(row,1)) end) + 1
    else
      1
    end
  end

  def getBagNumList(numbers, bags) do
    if numbers != nil do
      len = length(numbers) - 1
      for i <- 0..len do
        [Enum.at(numbers, i), Enum.at(bags, i)]
      end
    end
  end

  def getNumbersFromContain(contain) do
    if !String.contains?(contain, "no other bags") do
      numbers = contain |> String.replace( ~r/[[:alpha:]]/, "") |> String.replace(".", "") |> String.replace(" ","")
      num_list = numbers |> String.split(",")
      Enum.reduce(num_list, [], fn num, acc -> acc ++ [String.to_integer(num)] end)
    end
  end

  def getBagNamesFromContain(contain) do
    removed_num = contain |> String.replace(~r/[[:digit:]]/, "") |> String.replace(".", "") |> String.replace_leading(" ","")
    if String.contains?(removed_num, ",") do
      removed_num |> String.split(",") |> Enum.reduce([],fn bag, acc -> acc ++ [String.replace_leading(bag," ","")] end)
    else
      [removed_num]
    end
  end

  def checkContain(_map, "no other bags", _check) do
    false
  end

  def checkContain(map, bag_type, check) do
    contain = getContain(map, bag_type)

    if String.contains?(contain, check) do
      true
    else
      contain_bags = getBagNamesFromContain(contain)
      Enum.any?(contain_bags, fn bag -> checkContain(map, bag, check) end)
    end
  end

  def createMap(row, map) do
    split = String.split(row, "contain")
    bag = split |> Enum.at(0) |> String.replace_leading(" ","") |> String.replace_trailing(" ","")
    contains = split |> Enum.at(1)

    Map.put(map,bag,contains)
  end

  def parseData do
    data = "muted white bags contain 4 dark orange bags, 3 bright white bags.
    ...
    Enum.reduce(data, %{}, fn row, acc -> createMap(row, acc) end)
  end
end
